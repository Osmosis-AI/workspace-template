...
Run an evaluation from your machine with the rollout's configured `LocalBackend` or Harbor backend:
...
osmosis model list
osmosis model info <model-name>
osmosis model deploy <lora-model-name>
osmosis model undeploy <lora-model-name>
```

Deployed models serve an OpenAI-compatible API at `https://inference.osmosis.ai/v1` (`model` = `<base_model_path>:<lora-model-name>`, e.g. `Qwen/Qwen3-4B:code-reviewer-v1`, authenticated with an Osmosis API key as the bearer token). The model's detail page on the platform has ready-to-run snippets.
