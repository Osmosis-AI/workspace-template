...
- A registry dataset whose verifier reads its own credentials names them in `[verifier.env]`, mapping each variable to the Platform secret record supplying it (at most 16). Managed benchmarks model their credentials in the catalog and reject the section. A secret named there cannot also appear in top-level `[env]` or any `[agents.env]`.
