...
4. Run an evaluation locally with `osmosis --json eval run configs/eval/<name>.toml`, then optionally publish the completed result with `--upload`. Keep the environment the rollout configures: for Daytona, another Harbor cloud environment, or model-calling Harbor Docker on Linux, the run starts a `cloudflared` tunnel automatically, so keep `cloudflared` on `PATH`.
...
- Benchmark credential fields contain Platform secret record names. A provider or endpoint model's `[agents.model].api_key_secret` name cannot also appear in top-level `[env]` or that agent's `[agents.env]`, and cannot be `DAYTONA_API_KEY` or `DAYTONA_API_URL`, which are Platform-managed sandbox plumbing.
