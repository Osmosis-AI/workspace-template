...
- The `LLM Judge` row of `benchmark info` decides which `[execution]` judge fields a benchmark takes. `Required (default: <model>)` (HLE, GDPVal) needs `judge_api_key_secret`, with `judge_model` optional and defaulting to the benchmark's judge. `API key only (pinned grader)` (BrowseComp) needs `judge_api_key_secret` and rejects `judge_model`, because the adapter pins its own grader. `–` rejects both. Create the named record with `osmosis secret set <NAME>`; a judge secret name cannot appear in top-level `[env]` or any `[agents.env]`.
