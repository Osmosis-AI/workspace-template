...
4. Run an evaluation locally with `osmosis --json eval run configs/eval/<name>.toml`, then optionally publish the completed result with `--upload`. Keep the environment the rollout configures: for Daytona, SkyPilot, another Harbor cloud environment, or model-calling Harbor Docker on Linux, the run starts a `cloudflared` tunnel automatically, so keep `cloudflared` on `PATH`.
...
- Benchmark credential fields contain Platform secret record names. A provider or endpoint model's `[agents.model].api_key_secret` name cannot also appear in top-level `[env]` or that agent's `[agents.env]`, and cannot be `DAYTONA_API_KEY`, `DAYTONA_API_URL`, `SKYPILOT_SERVICE_ACCOUNT_TOKEN`, or `SKYPILOT_API_SERVER_ENDPOINT`, which are Platform-managed sandbox plumbing.
- Before editing a benchmark config, use `osmosis --json benchmark list` and `osmosis --json benchmark info <key>` to verify the benchmark's key and name, task sets, categories, harness and judge requirements, and the full task manifest. Every task's `difficulty` is `easy`, `medium`, `hard`, or `null`; `null` means the source did not provide a difficulty, and agents must never infer one.
...
- `cursor-cli` requires per-agent `harness_api_key_secret` set to `CURSOR_API_KEY`; any other value is rejected. That name cannot also appear in top-level `[env]` or the corresponding agent's `[agents.env]`. Mini SWE-agent rejects `harness_api_key_secret` for every model type. With provider and endpoint models, the Platform reuses the model's `api_key_secret` as `MSWEA_API_KEY`, so that name cannot also appear in the effective literal env; hosted models receive no injected model key and may set `MSWEA_API_KEY`. Omit `harness_api_key_secret` for every other harness.
- `benchmark info` reports `requires_judge_model` and `requires_judge_api_key`, and those two decide which `[execution]` judge fields a benchmark takes. Both true (HLE, GDPVal): `judge_api_key_secret` is required and `judge_model` is optional, defaulting to the benchmark's judge. Only `requires_judge_api_key` (BrowseComp): `judge_api_key_secret` is required and must hold an OpenAI key, and `judge_model` is rejected, because the adapter pins its own OpenAI grader. Both false: each field is rejected. Create the named record with `osmosis secret set <NAME>`; a judge secret name cannot appear in top-level `[env]` or any `[agents.env]`.
- A run lists under `[secrets].required` any credential that no other config field points at. Values are resolved at submit from `--secrets-file`, the process environment, the secret store, or an interactive prompt, first hit wins; never write a value into the TOML. A value supplied at submit is not saved and must be re-supplied on the next run. This applies to benchmark, training, and eval configs alike.
- A registry dataset whose verifier reads its own credentials names secret records in `[verifier].required` (at most 16); each is delivered under its own name. Managed benchmarks model their credentials in the catalog and reject the section. A secret named there cannot also appear in top-level `[env]` or any `[agents.env]`.
...
- `eval run` executes with the rollout's `LocalBackend` or Harbor backend and writes `.osmosis/evals/<run-name>/` by default. When the sandbox cannot reach this machine, as with Harbor cloud environments, the run starts a `cloudflared` tunnel to the local model bridge automatically, so keep `cloudflared` on `PATH`; optional `--tunnel cloudflared` only forces that tunnel, and `--listener-port <port> --advertise-url <url>` substitutes a public endpoint you manage. Omitting `--name` generates an `adjective-animal-number` name; pass that exact name to resume. Add `--upload` to publish only after the run reaches a complete terminal state; failed and skipped samples are terminal, while pending or cancelled runs cannot be uploaded.
- `eval upload <run-name-or-directory>` publishes an already-completed compatible local run without confirmation or extra flags. A bare run name resolves under the workspace's `.osmosis/evals/`; an explicit path is used as given. It requires workspace authentication/context and is safe to repeat after interruption: the server returns the same platform run and the CLI uploads only files still missing there.
