...
- A Harbor registry benchmark's task list pages in after it is added. `benchmark list` reports `sync_status`; submitting against anything other than `ready` fails. A `failed` row carries a `sync_error`, and its `platform_url` opens the benchmark's page — retry the sync from the benchmark catalog in the Platform.
