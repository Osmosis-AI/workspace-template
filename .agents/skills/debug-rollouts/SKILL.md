...
- Sample/reward contract: workflow bypasses Osmosis with direct fixed-model provider calls; the workflow registers zero or multiple sample sources instead of exactly one; the grader cannot read `ctx.sample` or skips `ctx.set_reward(...)`; reward logic is too strict, lenient, or broken.
