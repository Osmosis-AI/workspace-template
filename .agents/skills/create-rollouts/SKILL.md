...
Run an evaluation locally first to confirm the entrypoint, workflow, grader, model config, and selected local backend work together. For Harbor, manually set `environment_config.type` to `EnvironmentType.DOCKER` first; local eval does not support Daytona, SkyPilot, or other Harbor environments, so restore the usual setting afterward. The dataset can differ (e.g. a held-out eval set), but `[experiment].dataset` must be a platform dataset name, not a placeholder or local path. Uploading that result is optional and does not replace the managed full-size evaluation gate before training.
