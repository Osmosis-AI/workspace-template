...
Run an evaluation locally first to confirm the entrypoint, workflow, grader, model config, and selected backend work together. Keep the configured environment: for Daytona, another Harbor cloud environment, or model-calling Harbor Docker on Linux, `eval run` starts a `cloudflared` tunnel to the local model bridge automatically, so keep `cloudflared` on `PATH`. Daytona configs must list `DAYTONA_API_KEY` under `[secrets].required`. The dataset can differ (e.g. a held-out eval set), but `[experiment].dataset` must be a platform dataset name, not a placeholder or local path. Uploading that result is optional and does not replace the managed full-size evaluation gate before training.
