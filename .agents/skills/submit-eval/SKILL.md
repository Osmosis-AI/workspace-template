...
`eval upload` has no confirmation prompt. Its only flag is `--replace`, for republishing a run re-run with `--retry-failed`. A bare run name resolves under the workspace's `.osmosis/evals/`; a path with a separator, or an existing directory of that name, is used as given. It requires workspace authentication/context and a compatible directory containing `manifest.json`, `index.jsonl`, `progress.json`, and `metrics.json`. Re-running it after interruption is safe: the server returns the same platform run and the CLI uploads only files missing there.

The upload includes `index.jsonl`, `progress.json`, the combined `logs.txt` for the platform Logs tab, canonical referenced `trajectory*.json` files, and safe artifacts for selected rollout IDs. Configured secrets are redacted during logging; known ambient provider/platform credentials of at least eight characters available in the current process environment are redacted during logging and again before upload. The upload pass cannot redact a value no longer present in that environment; review logs before sharing. Uploads exclude local `manifest.json` bytes, `events.jsonl`, `metrics.json`, summary/projection copies, control files, per-trial logs, and superseded attempts. Manifest digest, schema versions, and allowlisted redacted Git provenance are sent as metadata; the server validates files, recomputes metrics, and does not launch a hosted or Temporal evaluation.
...
If any gate is missing or failing, route to `evaluate-rollouts` or `debug-rollouts` before retrying. Find run names with `osmosis --json eval list --limit 10`. Stop a run submitted by mistake with `osmosis --json eval stop <eval-name> --yes`. When a run's failed or skipped samples are infrastructure noise rather than a rollout defect, `osmosis --json eval retry <eval-name> --yes` re-runs only those samples under the same run and configuration; graded samples are carried forward untouched, so a retry never re-rolls a reward. It can still move the score: an ungraded sample counts against pass rate while carrying no reward, so grading it on a second attempt can only raise or hold that rate. Retry infrastructure noise, not a disappointing result. A rollout or grader change requires a fresh submission. If the run supplied secret values itself, the retry needs them again: the platform names them and the CLI resolves them from `--secrets-file`, the environment, or a prompt, so a non-interactive retry must pass `--secrets-file`.
